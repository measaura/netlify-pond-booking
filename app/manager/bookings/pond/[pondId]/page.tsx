...
export default function ManagerPondBookingsPage() {
  const params = useParams()
  const pondId = parseInt(params.pondId as string)
  
  const [pond, setPond] = useState(getPondById(pondId))
  const [bookings, setBookings] = useState<Booking[]>([])
  const [loading, setLoading] = useState(true)

  const loadPondBookings = () => {
    setLoading(true)
    try {
      const allBookings = getAllBookings()
      const pondBookings = allBookings.filter(booking => 
        booking.pond.id === pondId && booking.type === 'pond'
      )

      // Transform to match Booking interface
      const transformedBookings: Booking[] = pondBookings.map((dbBooking: BookingData) => {
        ...
        return {
          ...
          seats: seatLabels,
          instructions: 'Equipment rental available on-site. Check weather conditions before arrival.',
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50 pb-20">
          <div className="max-w-md mx-auto p-4">
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Pond Not Found</h3>
              <p className="text-gray-600 mb-4">The requested pond could not be found.</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {bookings.length} bookings • Capacity: {pond.capacity}
                    </p>
                  </div>
                </div>
              </div>
              <Button variant="ghost" size="sm" onClick={loadPondBookings}>
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">This pond has no bookings.</p>
              </div>
            ) : (
